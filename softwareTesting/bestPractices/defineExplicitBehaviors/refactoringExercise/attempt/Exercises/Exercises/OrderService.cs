...
public class Store
{
    ...
    public bool ExpressShippingAvailable => StoreLocation == StoreLocation.Warehouse || StoreLocation == StoreLocation.Mall;
}
...
public class OrderService
{
    ...
    public OrderService(Store store, List<Product> productInventory, List<Order> orders)
    {
        _productInventory = productInventory;
        _orders = orders;

        _storeLocation = store.StoreLocation;

        _expressShippingAvailable = store.ExpressShippingAvailable;

        Console.WriteLine($"Store {store.StoreLocation} initialized with express shipping: {_expressShippingAvailable}");
    }
    ...
    /* The function mutates state, so it should return a Result<T> for testability*/
