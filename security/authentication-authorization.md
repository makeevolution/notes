...
##### Where to store JWT
    ...
    - Storing in localstorage as Auth Bearer (like in section above) and making the frontend load it and send as header to backend has many differences over sending it over httponly cookies:
      ...
      - So using httponly cookie with samesite=None and Secure=True allows persistence (if the browser is closed and reopened you can still auto login), or for SSO, where the token can then be used from multiple frontends, but makes it more complicated since we need to implement CSRF protection. 
